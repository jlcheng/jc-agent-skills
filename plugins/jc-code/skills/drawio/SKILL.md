...
- **Output format** — the default deliverable is the `.drawio` file only; never produce a rendered image (PNG/SVG/PDF/JPG) the user didn't explicitly ask for (see **Default stop** below). Only ask this question when the user mentioned wanting an image.
...
*In the default `.drawio`-only flow, run this check against the ephemeral scratchpad preview and delete that PNG when done — it must never land in the user's directories or be presented as a deliverable.*
...
After self-check, show the exported image and ask the user for feedback. In the default `.drawio`-only flow, the review medium is draw.io desktop instead: the user opens the file, describes changes, and you apply the same targeted edit rules below — never export a PNG just to show progress.
...
- After applying edits, re-export and show the updated image (image-requested flow) — in the default flow, tell the user the file is updated so they can reload it in draw.io desktop
...
- Export to exactly the formats the user explicitly requested — if none, there is nothing to export: the `.drawio` file is the deliverable
...
Exports are **opt-in**: they run only in the image-requested flow (see **Default stop** in the Workflow). When they do run, there are **two** export modes:
